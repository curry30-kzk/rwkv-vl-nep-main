...
### [Optional] DeepSpeed ZeRO Stage-1

`run_nepa.py` uses Hugging Face `TrainingArguments`, so you can enable DeepSpeed via `--deepspeed <config.json>`.

We provide ZeRO Stage-1 configs:
- `configs/deepspeed/zero_stage1_bf16.json` (bf16)
- `configs/deepspeed/zero_stage1_fp16.json` (fp16)
- `configs/deepspeed/zero_stage1.json` (auto)
...
If you see `cuDNN error: CUDNN_STATUS_INTERNAL_ERROR`, try fp16 and/or enable gradient checkpointing:
```bash
MIXED_PRECISION=fp16 USE_DEEPSPEED=1 USE_GC=1 bash scripts/pretrain/rwkv_nepa_b.sh
```
...
## Merge ViT-SFT Head/FFN with RWKV TimeMix

If you have:
- a ViT-NEPA SFT checkpoint (embeddings + FFN + classification head), and
- a RWKV-NEPA trained checkpoint (TimeMix),

you can merge them into a RWKV-NEPA image-classification checkpoint:

```bash
python init_nepa_cls_from_vit_nepa_sft.py \
  --vit_nepa_sft_checkpoint weights/nepa/nepa_base_patch14_224_sft.safetensors \
  --rwkv_nepa_trained_checkpoint weights/rwkv_nepa_base_patch14_224_trained/model.safetensors \
  --rwkv_nepa_config_dir weights/rwkv_nepa_base_patch14_224_trained \
  --output_dir outputs/rwkv_nepa_base_patch14_224_cls_merged \
  --overwrite
```

Use `--dry_run` to validate the transfer without writing files.
